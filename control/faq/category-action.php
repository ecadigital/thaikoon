...
if($ModuleAction == "AddForm"){
	...
	$sql = "select *  from faq_category where cate_id = '$ModuleDataID'";
...
<?
		...
		$_order_by=SystemGetMaxOrder("faq_category","site_code='S0001'")+1;
		$insert["order_by"] 			= "'".$_order_by."'";
		
		$sql = "insert into faq_category(".implode(",",array_keys($insert)).") values (".implode(",",array_values($insert)).")";
		$Conn->execute($sql);
...
<? 
	...
	$sql = "update  faq_category set ".implode(",",array_values($update)) ;
...
<?
	...
	for($i=0;$i<$max; $i++){
		...
		$sql="UPDATE faq_category SET ".implode(",",$update)." WHERE cate_id='".$TmpArrID[$i]."' ";
		$Conn->execute($sql);
...
<?
	...
	$sql = "SELECT c.* FROM faq_category c";
	...
	if($CntParent>0){	
		...
		$sql = "delete from faq_category where cate_id = '".$cate_id."'";
		//echo $sql;
		$Conn->execute($sql);
