...
if($ModuleAction == "AddForm"){
	...
	$sql = "select *  from store_category where cate_id = '$ModuleDataID'";
...
<div class="controls">
<select id="parent_id" name="parent_id" class="request" required>
...
<?
		...
		$_order_by=SystemGetMaxOrder("store_category","site_code='S0001'")+1;
		$insert["order_by"] 			= "'".$_order_by."'";
		
		$sql = "insert into store_category(".implode(",",array_keys($insert)).") values (".implode(",",array_values($insert)).")";
		$Conn->execute($sql);
...
<? 
	...
	$sql = "update  store_category set ".implode(",",array_values($update)) ;
...
<?
	...
	$sql = "SELECT c.* FROM store_category c";
	...
	if($CntParent>0){	
		...
		$sql = "delete from store_category where cate_id = '".$cate_id."'";
		$Conn->execute($sql);
	...
	for($i=0;$i<$max; $i++){
		...
		$sql="UPDATE store_category SET ".implode(",",$update)." WHERE cate_id='".$TmpArrID[$i]."' ";
		$Conn->execute($sql);
