...
<select id="cate_id" name="cate_id" onchange="sysListCateIDSearch();" >
...
<a  _id="<?=$Row["id"]?>" href="javascript:void(0)" data-toggle="tooltip" title="" class="btn btn-mini btn-danger btn-del" data-original-title="Delete"><i class="icon-remove"></i></a></div>
</td>
</tr>
<? } ?>
</tbody>
...
$sql = "SELECT f.* FROM store_file f";
$sql.= " WHERE f.store_id='".$store_id."'";

$Conn->query($sql);
$ContentList = $Conn->getResult();
$CntRecInPage = $Conn->getRowCount();	
$libraryFolder = _SYSTEM_UPLOADS_FOLDER_.'/store'; 
...
$sql = "delete from store_file where store_id = '".$store_id."'";
$Conn->execute($sql);


$sql = "delete from store_main where id = '".$store_id."'";
$Conn->execute($sql);
