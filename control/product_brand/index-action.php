...
if ($ModuleAction == "Datalist") {
	...
	$sql = "SELECT p.*,c.name as category_name FROM products_brand p";
...
<? if($SysPMS=="MANAGE"){ ?>
<a href="index.php?<?=SystemEncryptURL("SysMenuID=$SysMenuID&ModuleAction=AddForm")?>" class="btn btn-success" ><i class="icon-plus"></i> Add New Brand</a>
...
<?
	...
	$Row = $ContentList[$i];
 	$physical_name="../../uploads/product_brand/".$Row["filepic"];
...
<?
	...
	if($input_fileIDOnePic!=""){
		...
		$storeFolder = _SYSTEM_UPLOADS_FOLDER_.'/product_brand';  
	...
	if($input_fileIDOnePicHome!=""){
		...
		$insert["date_display"] 			= "'".SystemDateFormatDB(trim($_REQUEST['input_showdate']))."'";
		...
		if($input_fileIDOnePicHome!=""){
			$insert["filepichome"] 			= "'".$store_physical_namehome."'";	
		...
		$_order_by = SystemGetMaxOrder("products_brand","menu_id='".$_REQUEST['SysMenuID']."'")+1;
		$insert["order_by"] 			= "'".$_order_by."'";
		
		
		$sql = "insert into products_brand(".implode(",",array_keys($insert)).") values (".implode(",",array_values($insert)).")";
		$Conn->execute($sql);
...
$input_content = stripslashes( (string)$_REQUEST["input_content"] );
	...
	if($input_fileIDOnePicHome!=""){
		$update[] = "filepichome 		= '".$store_physical_namehome."'";
	}
	...
	$sql = "update  products_brand set ".implode(",",array_values($update)) ;
	...
	if(count($file_delete)){
		 foreach($file_delete as $val){
			...
			$file_path_delete = _SYSTEM_UPLOADS_FOLDER_.'/product_brand/'.$row_filestore["physical_name"];
...
$sql = "SELECT f.filepic FROM products_brand f";
$sql.= " WHERE f.id='".$pid."'";

$Conn->query($sql);
$ContentList = $Conn->getResult();	
$Row = $ContentList[0];	
$libraryFolder = _SYSTEM_UPLOADS_FOLDER_.'/product_brand'; 
$physical_name=$Row["filepic"];

@unlink($libraryFolder."/".$physical_name);


$sql = "delete from products_brand where id = '".$pid."'";
$Conn->execute($sql);
...
<?
	...
	for($i=0;$i<$max; $i++){
		$update="";
		$update[]="order_by='".($i)."'";
		$sql="UPDATE products_brand SET ".implode(",",$update)." WHERE id='".$TmpArrID[$i]."' ";
