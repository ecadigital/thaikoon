...
<li class="active"><a href="../products/category.php?<?=SystemEncryptURL("SysMenuID=$SysMenuID")?>" >ชนิดสินค้า</a></li>
...
<div class="controls">
			...
			$ObjUFileID="OnePic";
			$ObjUFileType="onepic";
			$img="../../uploads/product_cate/".$Row["filepic"];
...
<?
	...
	for ($i=0;$i<$CntRecInPage;$i++) {
		...
		$Row = $ContentList[$i];
		$sql1 = "SELECT count(*) as CNT FROM products_category ";
...
<a id="btn_add" class="btn btn-success"  href="category.php?<?=SystemEncryptURL("SysMenuID=$SysMenuID&SysMenuID=$SysMenuID&ModuleAction=AddForm")?>"><i class="icon-plus"></i> Add New Category</a>
