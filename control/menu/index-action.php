...
$SourceModule["store"]="Product";
$SourceModule["store-cate"]="Product Category";
...
for ($i=0;$i<$CntRecInPage;$i++) {
	$RowCate = $ContentList[$i];
	$SourceProductCate["store-menucate-".$RowCate["cate_id"]]=$RowCate["name"];
}
...
<div id="area_product_cate" class="control-group <? if($chk_storecate<1){?> hide<? } ?>" >
    <label class="control-label" for="module_code">Product Category</label>
    <div class="controls">
    <select name="product_category" id="product_category"  onchange="sysListPerPage(this.value);">
  
     <?=SystemArraySelect($SourceProductCate,$Row["module_code"]);?>     		          
    </select>
    </div>
</div>
