...
<?
		...
		if($input_fileIDOnePic!=""){
			...
			$storeFolder = _SYSTEM_UPLOADS_FOLDER_.'/product_type';  
		...
		$_order_by=SystemGetMaxOrder("products_type","site_code='S0001'")+1;
		$insert["order_by"] 			= "'".$_order_by."'";
		
		$sql = "insert into products_type(".implode(",",array_keys($insert)).") values (".implode(",",array_values($insert)).")";
		$Conn->execute($sql);
...
<? 
	...
	$sql = "update  products_type set ".implode(",",array_values($update)) ;
...
<?
	...
	for($i=0;$i<$max; $i++){
		...
		$sql="UPDATE products_type SET ".implode(",",$update)." WHERE cate_id='".$TmpArrID[$i]."' ";
		$Conn->execute($sql);
